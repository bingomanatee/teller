(function () {
  ...
  app.controller('teller_map_edit', function ($scope, $window, $modal, $location, appearance, map, road, town, building, MAP_SIZE, MAP_ID, STORY_ID) {
    ...
    map.$scope = $scope;
    map.init_map($scope.size);

    // EASEL_MAP.grid_layer('back grid', map, {grid_params:{line_color: 'rgba(0, 204,0,0.5)'}});
    var _edit_url = _.template('/stories/<%= story %>/maps/<%= map %>/edit');

    $scope.edit_map = function () {
      document.location = _edit_url({story: STORY_ID, map: MAP_ID})
    }
